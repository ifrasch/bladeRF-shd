...
int nios_shd_write_params(struct bladerf *dev,
						uint16_t nint, uint32_t nfrac, uint8_t freqsel, uint8_t vcocap,
						bool low_band, uint8_t freq_index)
{
    int status;
    uint8_t buf[NIOS_PKT_LEN];
    bool success;
    ...
    //Pack request
	nios_pkt_shd_params_pack(buf, nint, nfrac, freqsel,
							vcocap, low_band, freq_index);
	...
	//Unpack response
    nios_pkt_shd_params_resp_unpack(buf, &success);
...
int nios_shd_write_control(struct bladerf *dev, bool run, uint8_t num_freqs)
{
    int status;
    uint8_t buf[NIOS_PKT_LEN];
    bool success;
    ...
    //Pack request
	nios_pkt_shd_control_pack(buf, run, num_freqs);
	//Send request and get response
    status = nios_access(dev, buf);
    if (status != 0) {
        return status;
	...
	//Unpack response
    nios_pkt_shd_control_resp_unpack(buf, &success);
