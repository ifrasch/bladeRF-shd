...
#include "trigger.h"
#include "smb_clock.h"
//Spectrum hole detection functions
#include "shd.h"
 ...
 * SHD routines
 *----------------------------------------------------------------------------*/
int bladerf_shd_send_params(struct bladerf *dev,
							uint32_t frequency,
							uint8_t freq_index)
{
    int status;
    ...
    status = shd_send_params(dev, frequency, freq_index);

    MUTEX_UNLOCK(&dev->ctrl_lock);
    return status;
}

int bladerf_shd_send_control(struct bladerf *dev,
						bool run,
						uint8_t num_freqs)
{
    int status;
    ...
    status = shd_send_control(dev, run, num_freqs);
